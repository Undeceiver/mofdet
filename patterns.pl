...
owl_satisfiable(A,exists(v1,TA)) :- class(A), TA =.. [A,tv1].

owl_not_connected_domain(P,A,lnot(exists(v1,and(TA,exists(v2,TP))))) :- property(P), class(A), TA =.. [A,tv1], TP =.. [P,tv1,tv2].
owl_not_connected_range(P,A,lnot(exists(v1,and(TA,exists(v2,TP))))) :- property(P), class(A), TA =.. [A,tv1], TP =.. [P,tv2,tv1].
...
extract_fault(unsatisfiable(X),unsatisfiable(X)).
